...
class Config: NSObject, NSCoding {
    ...
    convenience init?(dictionary: [String : AnyObject]) {
        ...
        /*if let imageDictionary = dictionary[VirtualTourist.Keys.ConfigImages] as? [String : AnyObject] {
            
            if let urlString = imageDictionary[VirtualTourist.Keys.ConfigBaseImageURL] as? String {
                baseImageURLString = urlString
            } else {return nil}
            
            if let secureUrlString = imageDictionary[VirtualTourist.Keys.ConfigSecureBaseImageURL] as? String {
                secureBaseImageURLString = secureUrlString
            } else {return nil}
            
            if let posterSizesArray = imageDictionary[VirtualTourist.Keys.ConfigPosterSizes] as? [String] {
                posterSizes = posterSizesArray
            } else {return nil}
            
            if let profileSizesArray = imageDictionary[VirtualTourist.Keys.ConfigProfileSizes] as? [String] {
    ...
    func updateIfDaysSinceUpdateExceeds(days: Int) {
        ...
        // Otherwise, update
        VirtualTourist.sharedInstance().updateConfig() { didSucceed, error in
            
            if let error = error {
                print("Error updating config: \(error.localizedDescription)")
            } else {
                print("Updated Config: \(didSucceed)")
                VirtualTourist.sharedInstance().config.save()
            }
        }
    }*/
